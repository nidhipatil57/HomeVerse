...
import { Calendar, Heart, MapPin, Users, Check } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Explore upcoming cultural celebrations, resident meetings, sports matches, and RSVP to confirm attendance
        </p>
      </div>
      ...
      <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {communityEvents.map((ev) => {
          const hasRsvped = ev.rsvps?.includes(user?.id || "user-resident-1");
          return (
            <motion.div key={ev.id} variants={fadeInUp}>
              <Card className="overflow-hidden border-border/50 hover:shadow-lg transition-all duration-300 flex flex-col h-full">
                <div className="aspect-[16/9] bg-gradient-to-br from-indigo-500/20 to-purple-500/20 relative flex items-center justify-center border-b border-border/20">
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  ...
                  <div className="mt-4 pt-4 border-t space-y-2 text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>{ev.rsvps?.length || 0} residents attending</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-5">
                    <Button
                      onClick={() => {
                        rsvpEvent(ev.id, user?.id || "user-resident-1");
                        alert(hasRsvped ? "RSVP cancelled." : "RSVP confirmed! See you there.");
                      }}
                    ...
                    >
                      {hasRsvped ? (
                        <span className="flex items-center justify-center gap-1.5"><Check className="w-4 h-4" /> Going</span>
                      ) : (
                        "RSVP to Event"
                      )}
                    </Button>
